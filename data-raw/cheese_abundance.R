...
cheese_taxonomy <-
  ...
  mutate(
    asv = if_else(asv >= 10, paste0("asv_", asv), paste0("asv_0", asv))
  ) %>%
...
cheese_abundance <-
  ...
  mutate(
    sample = str_remove_all(sample, "_ITS2_CURATED"),
    sample = str_replace_all(sample, "echantillon", "sample")
  ) %>%
  mutate(
    cheese = str_remove_all(sample, "sample"),
